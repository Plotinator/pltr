...
class ChapterPicker extends Component {
  ...
  renderTablet () {
    ...
    return <Popover
      from={<Button bordered dark iconRight style={styles.picker}><Text>{selected ? chapterHelpers.chapterTitle(selected, positionOffset, isSeries) : t('Select a Chapter')}</Text><Icon type='FontAwesome5' name='chevron-down' style={{fontSize: 12}}/></Button>}
    >
      <List>
  ...
  renderTabletItems () {
    const { chapters, positionOffset, isSeries, onChange, selectedId } = this.props
    return chapters.map(ch => {
      return <ListItem key={ch.id} style={styles.listItem} onPress={() => onChange(ch.id)} noIndent selected={ch.id == selectedId}><Text>{chapterHelpers.chapterTitle(ch, positionOffset, isSeries)}</Text></ListItem>
    })
  }
